        ...
        for n_workers, n_tasks in n_workers_tasks_list:
            ...
            if isinstance(anomaly_weights_matrix, list):
                ...
                for i in range(len(anomaly_weights_matrix)):
                    _, chose_edge = greedy_alg(anomaly_weights_matrix[i])
                    chose_edges.append(chose_edge)
            else:
                _, chose_edges = greedy_alg(anomaly_weights_matrix)
            ...
            res_df = pd.DataFrame({"dataset": dataset, "Methods": "greedy+outlier", "x": hp_valid_time, "workers": n_workers, "tasks": n_tasks,
                "weights": res, "avg_weights": avg_weights, "ratio(task)": ratio_task, "ratio(worker)": ratio_worker, "CPU time(s)": cpu_time,
                "normal_worker_ratio": normal_worker_ratio, "normal_task_ratio": normal_task_ratio, "normal_utility_ratio": normal_utility_ratio},
                index=[0])
            res_df.to_csv("results/{}anomaly_based_greedy_res_{}_x_{}_workers_{}_tasks_{}.csv".format(prefix, dataset, hp_valid_time, n_workers,
                n_tasks), index=False)
            print(res_df)
...
pd.DataFrame({"dataset": res_dataset_list, "Methods": "greedy+outlier", "x":res_hp_valid_time_list, "workers": res_workers_list, "tasks": res_tasks_list,
    ...
    res_normal_utility_ratio_list}).to_csv("results/{}anomaly_based_greedy_res_2468.csv".format(prefix), index=False)
