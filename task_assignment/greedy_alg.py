...
from utils import greedy_alg, get_normal_worker_task_pairs, get_weights_by_weightmatrix_and_edges
import pandas as pd
        ...
        for n_workers, n_tasks in n_workers_tasks_list:
            ...
            if isinstance(weights_matrix, list):
                ...
                for i in range(len(weights_matrix)):
                    _, chose_edge = greedy_alg(weights_matrix[i])
                    chose_edges.append(chose_edge)
            else:
                _, chose_edges = greedy_alg(weights_matrix)
            ...
            res_df = pd.DataFrame({"dataset": dataset, "Methods": "greedy", "x": hp_valid_time, "workers": n_workers, "tasks": n_tasks, "weights": res,
            ...
            res_df.to_csv("results/{}greedy_res_{}_x_{}_workers_{}_tasks_{}.csv".format(prefix, dataset, hp_valid_time, n_workers, n_tasks), index=False)
            print(res_df)
...
pd.DataFrame({"dataset": res_dataset_list, "Methods": "greedy", "x":res_hp_valid_time_list, "workers": res_workers_list, "tasks": res_tasks_list,
    ...
    res_normal_utility_ratio_list}).to_csv("results/{}greedy_res_2468.csv".format(prefix), index=False)
