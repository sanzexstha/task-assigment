...
for dataset in dataset_list:
    for hp_valid_time in hp_valid_time_list:
        ...
        for n_workers, n_tasks in n_workers_tasks_list:
            ...
            weights_matrix, _ = pickle.load(weights_matrix_path.open("rb"))

            start_time = time.time()
            if isinstance(weights_matrix, list):
                px_list = []
                for i in range(len(weights_matrix)):
                    _, px = KM(weights_matrix[i])
                    px_list.append(copy.copy(px))
                px = None
            else:
                _, px = KM(weights_matrix)

            end_time = time.time()
            cpu_time = end_time - start_time

            def convert_px_to_chose_edge(px, weights_matrix):
                N = weights_matrix.shape[0]
                chose_edge = []
                for i in range(1, N+1):
                    if weights_matrix[i-1][px[i]-1] != 0:
            ...
            if isinstance(weights_matrix, list):
                ...
                for i in range(len(px_list)):
                    chose_edges.append(convert_px_to_chose_edge(px_list[i], weights_matrix[i]))
            else:
                chose_edges = convert_px_to_chose_edge(px, weights_matrix)
            ...
            print("n_valid_task_worker_pairs: {}, n_tasks: {}, n_workers: {}, ratio_div: {}".format(n_valid_task_worker_pairs, n_tasks, n_workers, ratio_div))
            print("n_normal_workers: {}, n_normal_tasks: {}".format(n_normal_workers, n_normal_tasks))
            print("total_worker_task: {}".format(total_worker_task))
            ...
            res_df = pd.DataFrame({"dataset": dataset, "Methods": "KM", "x": hp_valid_time, "workers": n_workers, "tasks": n_tasks, "weights": res,
            ...
            res_df.to_csv("results/{}KM_res_{}_x_{}_workers_{}_tasks_{}.csv".format(prefix, dataset, hp_valid_time, n_workers, n_tasks), index=False)
            print(res_df)
...
pd.DataFrame({"dataset": res_dataset_list, "Methods": "KM", "x":res_hp_valid_time_list, "workers": res_workers_list, "tasks": res_tasks_list,
    ...
    res_normal_utility_ratio_list}).to_csv("results/{}KM_res_CPUTime.csv".format(prefix), index=False)
