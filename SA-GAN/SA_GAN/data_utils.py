...
def read_train_data_seq_win_based(seq_length, file = '', step=1, valid_portition=0.3, delta=0.05, ratio=0.01):
    ...
    #
    df1 = np.load('./datasets/train/' + file, allow_pickle=True).astype(float)
    #print(df1.shape)
    ...
    #print('ratio of negative samples:', ratio)
    #
    df = np.load('./datasets/train/' + file, allow_pickle=True).astype(float)
    #print(df.shape)
...
def read_test_data(seq_length, file = ''):
    ...
    label = np.load('./datasets/test_label/' + file, allow_pickle=True).astype(float)
